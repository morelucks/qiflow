    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card className="space-y-4">
        ...
        {profile?.walletAddress && (
          <div className="p-3 text-xs rounded-lg bg-gray-50 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300">
            Current address is a <span className="font-semibold">{addressLedger(profile.walletAddress)} address</span> — it can receive{' '}
            <span className="font-semibold">{addressLedger(profile.walletAddress)}</span> payments only. Quai Network encodes the
        ...
        <form onSubmit={saveWallet} className="space-y-3">
          ...
          {wallet.trim() && WALLET_RE.test(wallet.trim()) && (
            <p className="text-[11px] text-gray-500">
              Detected: {addressLedger(wallet.trim())} address → accepts {addressLedger(wallet.trim())}
            </p>
          )}
          {walletMsg && (
